...
trace from the committed bake (case-results.json) + the learned-model metrics (cc-learned.json, when present), runs
...
- see cclab/science/.
    ...
    python -m cclab.pipeline                 # rebuild all replay traces + manifests from committed artifacts
    python -m cclab.pipeline K-BALL          # one case
    python -m cclab.pipeline all --retrain   # re-bake case-results + train the learned models, then rebuild
"""
...
from .cases.mill_cases import descriptor_row
...
def _load_artifacts() -> tuple[dict, dict | None]:
    ...
    if not cr.exists():
        raise SystemExit(
            ...
            f"(science/bake_cases.mjs) - run `python -m cclab.pipeline all --retrain` (or `npm run bake` in frontend/)."
        )
    learned_path = DERIVED / "cc-learned.json"
    learned = read_json(learned_path) if learned_path.exists() else None
...
def retrain(seed: int = 42) -> None:
    """HEAVY lane (two-language): re-bake the mill evaluation (the SAME TS engine) and train the learned models
    (torch -> ONNX). The science is preserved verbatim in cclab/science/."""
    print("[retrain] bake case-results (TS mill engine over the cases) ...", flush=True)
    _node(str(SCIENCE / "bake_cases.mjs"))
    train = SCIENCE / "train_mill.py"
    if train.exists():
        print("[retrain] generate the learned-model training data (the SAME TS engine) ...", flush=True)
        _node(str(SCIENCE / "gen_train.mjs"))
        print("[retrain] torch train the learned models (power surrogate + OOD-AE) -> ONNX ...", flush=True)
        ...
        print("[retrain] eval the trained surrogate downstream (vs the exact engine) ...", flush=True)
        _node(str(SCIENCE / "eval_mill.mjs"))
    else:
        print("[retrain] (science/train_mill.py absent - learned models pending; traces record learned=pending)", flush=True)
...
def main() -> None:
    ap = argparse.ArgumentParser(prog="cclab.pipeline")
    ...
    if args.case == "all":
        ...
        print(f"precomputed {args.case}: lane={m['lane']} bytes={m['artifact']['bytes']} metrics={m['metrics']} -> {DERIVED / m['artifact']['path']}")
