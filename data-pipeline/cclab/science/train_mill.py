...
assembles the final data/derived/cc-learned.json). Deterministic (seeded).
"""
...
# torch's ONNX exporter prints status with emoji; force UTF-8 stdout so it never crashes on a Windows cp1252 console
# (the pipeline.retrain subprocess path inherits whatever encoding the parent has).
...
# ----------------------------------------------------------------------------------------------------------------
# the power / frac-centrifuging surrogate
# ----------------------------------------------------------------------------------------------------------------
...
def main() -> None:
    d = json.loads((RAW / "mill-train.json").read_text())
    X = np.asarray(d["x"], dtype=np.float64)
    Y = np.asarray(d["y"], dtype=np.float64)
    ev = json.loads((RAW / "mill-eval.json").read_text())
    ...
    partial = {
        ...
        "honesty": ("Synthetic but physically-realistic mill operating points across a typical SAG/ball/rod envelope; "
                    ...
                    "engine on held-out points) is measured by eval_mill.mjs. The OOD autoencoder flags operating points "
                    "outside the envelope (over-speed, near-centrifuging, extreme geometry). The exact engine is the "
    ...
    print(f"wrote power-surrogate.onnx + scenario-ood.onnx + learned-partial.json -> {DERIVED} / {RAW}")
