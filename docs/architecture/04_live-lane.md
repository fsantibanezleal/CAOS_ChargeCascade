...
| `charge.ts` | the Davis (1919) per-shell departure: a charge element on a radial shell at radius `r` departs where `cos α = ω²r/g = φc²·(r/R)`, then flies a parábola `y = x·tanα − g·x²/(2v²cos²α)` to the toe, the outer shells thrown highest (the cataract fan); the shoulder / toe angles and `fracCentrifuging` (the fraction of shells with `cos α ≥ 1`) |
