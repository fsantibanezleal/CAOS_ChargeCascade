...
runs `python data-pipeline/run.py all` to regenerate the artifacts deterministically, builds the SPA (`copy-data.mjs`
...
(`python data-pipeline/run.py K-BALL`), ruff, pytest, and `npm run build`, plus `check_artifacts.py` (CONTRACT 2) and the
