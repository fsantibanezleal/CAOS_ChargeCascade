...
from cclab import pipeline


def test_manifest_matches_artifact_and_gate():
    m = pipeline.precompute("K-BALL", seed=7)
    ...
    assert m["schema"].startswith("chargecascade.manifest/")
    ...
    assert "phf_kw" in m["metrics"] and "regime" in m["metrics"]
