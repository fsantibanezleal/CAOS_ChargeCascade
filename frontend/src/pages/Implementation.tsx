  ...
  const ArchSVG = (
    <svg viewBox="0 0 920 520" width="100%" role="img" aria-labelledby="cc-arch-t cc-arch-d"
      ...
      <desc id="cc-arch-d">{es
        ? 'Un motor TypeScript exacto se ejecuta en vivo en el navegador (incluido el molino 3D) y, sin cambios, en el precálculo offline. Un carril torch→ONNX entrena el surrogate de potencia y el autoencoder OOD. Solo archivos estáticos viajan al CDN.'
      ...
      <text x="36" y="462" fontSize="10.5" fill="var(--color-fg-subtle)">{es ? '· el navegador descarga el bundle una vez y ejecuta el motor + ONNX localmente; misma entrada → misma salida' : '· the browser fetches the bundle once and runs the engine + ONNX locally; same input → same output'}</text>
  ...
  const tabs = [
    // 1, COMPUTE TIERS
    {
      ...
      content: (
        <div className="prose">
          ...
          <p>{es
            ? 'El contrato entre el navegador en vivo y el precálculo offline es que ejecutan el mismo motor. El precalculado importa el motor TypeScript exacto sin cambios y evalúa cada caso; el navegador importa el mismo motor y lo evalúa en vivo. Por eso los números que se ven en el App, en Experimentos y en los artefactos son idénticos por construcción, no por coincidencia.'
            : 'The contract between the live browser and the offline bake is that they run the same engine. The bake imports the exact TypeScript engine unchanged and evaluates each case; the browser imports the same engine and evaluates it live. That is why the numbers shown in the App, in Experiments and in the artifacts are identical by construction, not by coincidence.'}</p>
    ...
    // 3, THE 3D CHARGE
    {
      ...
      content: (
        <div className="prose">
          <p>{es
            ? 'El molino 3D (Three.js) anima exactamente las trayectorias que el motor computa: cada partícula sube con la pared, parte en su ángulo de Davis (cos α = φc²·r/R) y vuela una parábola balística hasta reimpactar en el toe. No es una animación decorativa, es la solución cinemática del motor, renderizada. Al subir la velocidad se ve el abanico de cataract abrirse; al ponerla en φc = 1 la carga se pega a la pared (centrifugado).'
            : 'The 3D mill (Three.js) animates exactly the trajectories the engine computes: each particle rides up with the wall, departs at its Davis angle (cos α = φc²·r/R) and flies a ballistic parabola until it re-impacts at the toe. It is not a decorative animation, it is the engine\'s kinematic solution, rendered. Raise the speed and the cataract fan opens; set φc = 1 and the charge pins to the wall (centrifuging).'}{' '}<Cite id="davis1919" paren /></p>
          <p>{es
          ...
          <Callout variant="honest" title={es ? 'Dos vistas: Davis cinemático (en vivo) y DEM real (horneado)' : 'Two views: Davis kinematic (live) and real DEM (baked)'}>
            {es ? 'La vista Davis es un modelo cinemático de partículas independientes siguiendo física publicada (Davis + parábola): correcta como envolvente de trayectorias, en vivo con cada deslizador, no un solve de N cuerpos. La vista DEM sí es un solve de elementos discretos real (milldem: colisiones, fricción de Coulomb, cadenas de fuerza), horneado offline porque el DEM no corre en el navegador; se reproducen las posiciones por cuadro. Su potencia neta está validada dentro de ~10-20% de Hogg-Fuerstenau y es consistente con el tamaño del molino.' : 'The Davis view is a kinematic model of independent particles following published physics (Davis + parabola): correct as a trajectory envelope, live with every slider, not an N-body solve. The DEM view IS a real discrete-element solve (milldem: collisions, Coulomb friction, force chains), baked offline because DEM cannot run in the browser; the per-frame positions are replayed. Its net power is validated within ~10-20% of Hogg-Fuerstenau and is size-consistent.'}
          </Callout>
    ...
    // 4, POWER COMPUTATION
    {
      ...
      content: (
        <div className="prose">
          ...
          <p>{es
            ? 'La magnitud necesita una constante de calibración c que absorbe la fricción del cojinete, la eficiencia del motor y la geometría del lifter. El motor la fija a una referencia industrial real (~1.3 MW para el molino de bolas de 4×6 m), así que las potencias mostradas son realistas. Es honesto: c es un ancla, no una predicción ciega, y junto a Hogg–Fuerstenau se ejecuta una forma Morrell calibrada (un reescalado del mismo brazo de torque, no un modelo independiente) como compañera de consistencia. El C-model real de Morrell, validado a ±9.8% sobre 82 sets industriales, está citado pero no implementado.'
          ...
          <p>{es
            ...
            : 'Bond\'s law is computed separately as a consumption sanity-check (W = 10·Wi·(1/√P80 − 1/√F80), kWh/t), it is not the charge power; it is the specific process energy to reduce from F80 to P80, and it does not animate with φc. The App keeps the two visibly distinct so power is not confused with energy.'}{' '}<Cite id="bond1952" paren /></p>
          <Callout variant="honest" title={es ? 'Forma exacta, magnitud calibrada' : 'Exact shape, calibrated magnitude'}>
            {es ? 'La forma de la potencia (el término de llenado, el escalado D^2.5, la dependencia de φc) es exacta y publicada; la magnitud está anclada a ~1.3 MW. El motor no atenúa el modelo de torque al centrifugar, a φc = 1 la potencia de tracción sigue alta; lo que colapsa es la molienda. Por eso C-CRITICAL (φc ≥ 1 → fracción centrifugando > 0, régimen = centrifuging) se verifica sobre la fracción/el régimen, y C-EMPTY (J = 0 → 0 potencia) es el control de potencia-cero. Ambos son controles analíticos exactos que el motor debe reproducir.' : 'The power shape (the fill term, the D^2.5 scaling, the φc dependence) is exact and published; the magnitude is anchored to ~1.3 MW. The engine does not taper the torque model when centrifuging, at φc = 1 the draw power stays high; what collapses is the grinding. So C-CRITICAL (φc ≥ 1 → centrifuging fraction > 0, regime = centrifuging) is verified on the fraction/regime, and C-EMPTY (J = 0 → 0 power) is the zero-power control. Both are exact analytic controls the engine must reproduce.'}
          </Callout>
          <Refs ids={['hoggfuerstenau1972', 'morrell1996', 'bond1952']} label={refsL} />
        </div>
      ),
    },
    // 5, LEARNED MODELS (ONNX LANE)
    {
      ...
      content: (
        <div className="prose">
          <p>{es
            ? 'Dos modelos aprendidos se entrenan offline (torch) y se exportan a ONNX para ejecutarse en vivo en el navegador (onnxruntime-web). El primero es un surrogate de potencia: un MLP que mapea las features del molino directamente a la potencia, entrenado para reproducir el motor exacto. Su rol previsto son los barridos masivos (Monte-Carlo, mapas (φc, J)); ninguna pestaña publicada ejecuta aún ese barrido, las pestañas en vivo usan el motor exacto sub-milisegundo, y la pestaña What-if compara el surrogate contra él. Su error held-out (~±12.5%) se reporta siempre, junto a la potencia exacta.'
    ...
    // 6, DATA CONTRACTS
    {
      ...
      content: (
        <div className="prose">
          ...
          <p>{es
            ? 'El contrato es lo que permite el modo de molino propio: un usuario puede ingresar un SAG, un molino de bolas o uno de barras reales y el motor lo evalúa con la misma física, siempre que pase las guardas de rango. La marca real-o-sintético es obligatoria, así cada caso declara honestamente si es un molino industrial publicado o una configuración ilustrativa.'
  ...
  return (
    <div className="page-body">
      <div className="page-head prose">
        ...
        <p className="lede">{es
          ? 'Un motor TypeScript exacto se ejecuta en vivo en el navegador (incluido el 3D) y, sin cambios, en el precálculo offline → artefactos compactos. Un carril torch→ONNX produce el surrogate de potencia + el autoencoder OOD. Todo determinista, todo reproducible.'
