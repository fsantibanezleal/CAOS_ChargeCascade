  ...
  const ArchSVG = (
    <svg viewBox="0 0 920 520" width="100%" role="img" aria-labelledby="cc-arch-t cc-arch-d"
      ...
      {[[es ? 'velocidad crítica · φc · ω' : 'critical speed · φc · ω', 104],
        ...
        [es ? 'potencia Hogg–Fuerstenau · Morrell' : 'Hogg–Fuerstenau power · Morrell', 176]].map(([t, y], i) => (
      ...
      <text x="36" y="316" fontSize="10.5" fill="var(--color-fg)">{es ? 'resultados de casos (JSON) · trazas + manifests · surrogate.onnx · ood-ae.onnx · errores held-out' : 'case results (JSON) · traces + manifests · surrogate.onnx · ood-ae.onnx · held-out errors'}</text>
  ...
  const tabs = [
    ...
    // 2 — THE MILL ENGINE
    {
      ...
      content: (
        <div className="prose">
          <p>{es
            ? 'El corazón es una función pura: dada una operación de molino (geometría D, L; llenado J; tamaño de bola d; fracción de velocidad crítica φc; densidad de carga) devuelve un resultado completo —velocidad crítica, ω, los ángulos de partida por capa, las trayectorias, el régimen, la fracción centrifugando, la potencia y la capacidad. Es la ÚNICA fuente de verdad física; todo lo demás (el 3D, las pestañas, el horneado) la consume.'
            : 'The heart is a pure function: given a mill operation (geometry D, L; fill J; ball size d; fraction of critical speed φc; charge density) it returns a complete result — critical speed, ω, the per-shell departure angles, the trajectories, the regime, the centrifuging fraction, the power and the capacity. It is the SINGLE source of physics truth; everything else (the 3D, the tabs, the bake) consumes it.'}</p>
          <p>{es
          ...
          <Equation tex="\texttt{evaluate}(op)\to\{N_c,\ \varphi_c,\ \alpha(r),\ \text{regime},\ f_{\text{cent}},\ P,\ \text{capacity}\}" caption={es ? 'la firma del motor: una operación de molino entra, un resultado físico completo sale' : 'the engine signature: a mill operation in, a complete physics result out'} />
          <ul>
    ...
    // 3 — THE 3D CHARGE
    {
      ...
      content: (
        <div className="prose">
          ...
          <Equation tex="\vec{r}(t)=\begin{cases}\text{pared, sube con }\omega & t<t_{\text{partida}}\\[2pt] \text{parábola: } y=x\tan\alpha-\dfrac{g\,x^2}{2v^2\cos^2\alpha} & t\ge t_{\text{partida}}\end{cases}" caption={es ? 'la trayectoria de cada partícula 3D: arrastre por la pared, luego vuelo libre desde el ángulo de partida' : 'each 3D particle\'s trajectory: wall drag, then free flight from the departure angle'} />
    ...
    // 4 — POWER COMPUTATION
    {
      ...
      content: (
        <div className="prose">
          ...
          <p>{es
            ? 'La MAGNITUD necesita una constante de calibración c que absorbe la fricción del cojinete, la eficiencia del motor y la geometría del lifter. El motor la fija a una referencia industrial real (~1.3 MW para el molino de bolas de 4×6 m), así que las potencias mostradas son realistas. Es honesto: c es UN ancla, no una predicción ciega — y el modelo de Morrell corre en paralelo como cross-check independiente (±9.8% sobre 82 sets), de modo que el lector ve que dos modelos coinciden.'
            : 'The MAGNITUDE needs a calibration constant c absorbing bearing friction, motor efficiency and lifter geometry. The engine fixes it to a real industrial reference (~1.3 MW for the 4×6 m ball mill), so the displayed powers are realistic. It is honest: c is ONE anchor, not a blind prediction — and Morrell\'s model runs alongside as an independent cross-check (±9.8% over 82 sets), so the reader sees two models agree.'}{' '}<Cite id="morrell1996" paren /></p>
          <p>{es
          ...
          <Callout variant="honest" title={es ? 'Forma exacta, magnitud calibrada' : 'Exact shape, calibrated magnitude'}>
            {es ? 'La forma de la potencia (el término de llenado, el escalado D^2.5, la dependencia de φc) es exacta y publicada; la magnitud está anclada a ~1.3 MW. C-CRITICAL (φc=1 → la potencia colapsa porque la carga centrifuga) y C-EMPTY (J=0 → 0 potencia) son controles analíticos exactos que el motor DEBE reproducir.' : 'The power shape (the fill term, the D^2.5 scaling, the φc dependence) is exact and published; the magnitude is anchored to ~1.3 MW. C-CRITICAL (φc=1 → power collapses because the charge centrifuges) and C-EMPTY (J=0 → 0 power) are exact analytic controls the engine MUST reproduce.'}
          </Callout>
    ...
    // 5 — LEARNED MODELS (ONNX LANE)
    {
      ...
      content: (
        <div className="prose">
          <p>{es
            ? 'Dos modelos aprendidos se entrenan offline (torch) y se exportan a ONNX para correr EN VIVO en el navegador (onnxruntime-web). El primero es un surrogate de potencia: un MLP que mapea las features del molino directamente a la potencia, entrenado para reproducir el motor exacto. Existe para los barridos masivos (Monte-Carlo, mapas (φc, J)) donde 10⁴ evaluaciones exactas serían lentas. Su error held-out (~±12.5%) se reporta siempre, junto a la potencia exacta.'
            : 'Two learned models train offline (torch) and export to ONNX to run LIVE in the browser (onnxruntime-web). The first is a power surrogate: an MLP mapping the mill features directly to power, trained to reproduce the exact engine. It exists for the mass sweeps (Monte-Carlo, (φc, J) maps) where 10⁴ exact evaluations would be slow. Its held-out error (~±12.5%) is always reported, alongside the exact power.'}</p>
          <Equation tex="\hat P=\text{MLP}_\theta(x),\qquad \theta^\*=\arg\min_\theta\tfrac1N\textstyle\sum_i(\text{MLP}_\theta(x_i)-P_i^{\text{exacto}})^2" caption={es ? 'el surrogate se entrena a reproducir la potencia del motor exacto sobre las features físicas x' : 'the surrogate trains to reproduce the exact engine power over the physical features x'} />
          <p>{es
          ...
          <Equation tex="\text{OOD}(x)=\lVert x-\text{AE}(x)\rVert^2,\qquad \text{marca si }\text{OOD}(x)>\theta_{p99}\quad(\text{AUC}\approx0.92)" caption={es ? 'el puntaje OOD es el error de reconstrucción; sobre el umbral p99 se marca el molino' : 'the OOD score is the reconstruction error; above the p99 threshold the mill is flagged'} />
          <Callout variant="honest" title={es ? 'Herramientas, no reemplazos' : 'Tools, not replacements'}>
            {es ? 'El motor exacto corre en vivo por defecto y es siempre la autoridad. El surrogate (~±12.5%) acelera los barridos; el AE (AUC ~0.92) marca cuándo el surrogate extrapola. Ambos errores se reportan tal cual — sin victorias fabricadas, sin presentar un estimador rápido como verdad.' : 'The exact engine runs live by default and is always the authority. The surrogate (~±12.5%) accelerates the sweeps; the AE (AUC ~0.92) flags when the surrogate extrapolates. Both errors are reported as they land — no fabricated wins, no presenting a fast estimator as truth.'}
          </Callout>
    ...
    // 7 — VERIFICATION
    {
      ...
      content: (
        <div className="prose">
          <p>{es
            ? 'El motor se verifica contra oráculos analíticos —resultados conocidos de la física, no auto-referencias. Diez oráculos cubren: la velocidad crítica (Nc = 42.3/√(D−d) para un molino de referencia), la partida a φc = 1 (toda la carga centrifuga), el pico de potencia en J ≈ 0.47, el escalado P ∝ D^2.5 (duplicar D multiplica la potencia por ~5.7), la magnitud realista (~1.3 MW de referencia) y la ley de Bond.'
            : 'The engine is verified against analytic oracles — known physics results, not self-references. Ten oracles cover: the critical speed (Nc = 42.3/√(D−d) for a reference mill), the φc = 1 departure (all charge centrifuges), the J ≈ 0.47 power peak, the P ∝ D^2.5 scaling (doubling D multiplies power by ~5.7), the realistic magnitude (~1.3 MW reference) and Bond\'s law.'}</p>
          <Equation tex="\text{C-CRITICAL: }\varphi_c=1\Rightarrow P\to0,\qquad \text{C-EMPTY: }J=0\Rightarrow P=0,\qquad \frac{P(2D)}{P(D)}\approx2^{2.5}\approx5.7" caption={es ? 'tres de los oráculos exactos que la verificación exige al motor' : 'three of the exact oracles the verification holds the engine to'} />
          <p>{es
