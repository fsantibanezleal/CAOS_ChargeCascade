      ...
      content: (
        <div className="prose">
          <p>{es
            ...
            : 'This page does not report a single headline number; it separates the questions the engine can be made to FAIL. (1) Do the three independent power models (Hogg–Fuerstenau, Morrell, Bond) agree with each other? (2) Does the regime transition land where physics says, when φc is swept over the SAME mill? (3) Do the exact analytic controls hold (φc ≥ 1 → the centrifuging fraction turns positive and the regime switches to centrifuging; J = 0 → 0 power)? (4) Does the learned surrogate generalize to unseen mills, and does the AE flag when it must not be trusted?'}</p>
          <p>{es
    ...
    // 4 — EXACT ANALYTIC CONTROLS
    {
      ...
      content: (
        <div className="prose">
          ...
          <Equation tex="\text{C-CRITICAL: }\varphi_c\ge1\Rightarrow f_{\text{cent}}>0,\ \text{régimen}=\text{centrifuging (molienda colapsa)};\qquad \text{C-EMPTY: }J=0\Rightarrow M=0\Rightarrow P=0" caption={es ? 'los dos controles exactos: C-CRITICAL se verifica sobre la fracción/el régimen (no la potencia); C-EMPTY sobre la potencia cero' : 'the two exact controls: C-CRITICAL is verified on the fraction/regime (not the power); C-EMPTY on zero power'} />
          <p>{es
