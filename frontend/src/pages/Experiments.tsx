  ...
  const tabs = [
    ...
    // 2, CROSS-METHOD POWER AGREEMENT
    {
      ...
      content: (
        <div className="prose">
          ...
          <p>{es
            ? 'El escalado con el tamaño es la prueba real: la teoría exige P ∝ D^2.5, así que duplicar el diámetro debe multiplicar la potencia por ~5.7. K-SAG (10 m) saca la mayor potencia de los 10 casos por exactamente esa razón, y la tabla de casos lo hace verificable fila a fila. Bond se calcula como un cruce aparte, pero de energía (kWh/t), no de potencia, deliberadamente mantenido aparte para no confundir las dos cantidades.'
    ...
    // 6, CASES + CAPACITY (the live table)
    {
      ...
      content: (
        <div className="prose">
          <p>{es
            ? 'Los 10 casos, precalculados por el mismo motor exacto que se ejecuta en vivo. Cada fila lleva la velocidad crítica, φc, el régimen, la potencia neta, el % centrifugando y la masa de carga, los números son idénticos a los que el App muestra al seleccionar el caso, por construcción.'
            : 'The 10 cases, baked by the same exact engine that runs live. Each row carries the critical speed, φc, the regime, the net power, the % centrifuging and the charge mass, the numbers are identical to what the App shows when the case is selected, by construction.'}</p>
