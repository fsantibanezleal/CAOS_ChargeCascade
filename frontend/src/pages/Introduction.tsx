...
export default function Introduction() {
  ...
  return (
    <article className="page-body prose">
      ...
      <figure className="cc-fig">
        <svg viewBox="0 0 880 250" width="100%" role="img" aria-labelledby="cc-ov-t cc-ov-d" style={{ display: 'block', fontFamily: 'var(--font-sans, system-ui, sans-serif)' }}>
          ...
          <g className="cc-ov">
            ...
            <text className="s" x="656" y="68">{es ? 'animación cinemática de la carga' : 'kinematic charge animation'}</text>
            <text className="s" x="656" y="88">{es ? '(NO un solve DEM)' : '(NOT a DEM solve)'}</text>
            <text className="s" x="656" y="112">{es ? 'surrogate de potencia (sweeps)' : 'power surrogate (sweeps)'}</text>
      ...
      <p>{es
        ? 'El problema es que la carga es INVISIBLE: vive dentro de un tambor de acero girando. El ingeniero de molienda no puede verla, así que razona sobre ella con modelos. ChargeCascade hace visible ese razonamiento — el mismo que está detrás del dimensionamiento de molinos, la selección de revestimientos y la optimización de velocidad variable.'
        : 'The problem is that the charge is INVISIBLE: it lives inside a rotating steel drum. The grinding engineer cannot see it, so they reason about it with models. ChargeCascade makes that reasoning visible — the same reasoning behind mill sizing, liner selection and variable-speed optimisation.'}</p>
      ...
      <Equation tex={String.raw`N_c=\frac{42.3}{\sqrt{D-d}}\ \text{rpm},\qquad \varphi_c=\frac{N}{N_c}`} caption={es ? 'velocidad crítica (D = diámetro interno del molino en m, d = diámetro de bola en m) y la fracción de velocidad crítica φc; la constante 42.3 = 60·√(g)/(2π·√2) es derivada, no ajustada' : 'critical speed (D = mill inside diameter in m, d = ball diameter in m) and the fraction of critical speed φc; the constant 42.3 = 60·√(g)/(2π·√2) is derived, not fitted'} />
      ...
      <Equation tex={String.raw`P=\omega\,M\,g\,R_{\text{arm}}\sin\theta,\qquad M\propto J(1-1.065\,J)\,\rho_c` } caption={es ? 'P = potencia; ω = velocidad angular; M = masa de carga (∝ término de llenado · densidad de carga ρc); R_arm·sinθ = brazo de torque al centroide de la carga; cross-check con Morrell (1996)' : 'P = power; ω = angular speed; M = charge mass (∝ fill term · charge density ρc); R_arm·sinθ = torque arm to the charge centroid; cross-checked with Morrell (1996)'} />
      ...
      <ol>
        ...
        <li>{es ? 'Calcular la potencia (Hogg–Fuerstenau, cross-check Morrell) y la capacidad.' : 'Compute the power (Hogg–Fuerstenau, Morrell cross-check) and the capacity.'}</li>
