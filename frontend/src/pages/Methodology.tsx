        ...
        // ============================================================ CHARGE MOTION (DAVIS)
        {
          ...
          content: (
            <div className="cc-doc-sec">
              ...
              <Equation tex="y = x\tan\alpha - \frac{g\,x^2}{2v^2\cos^2\alpha},\qquad v=\omega r" caption={es ? 'la parábola de vuelo libre desde el punto de partida; x, y relativos al punto de partida' : 'the free-flight parábola from the departure point; x, y relative to the departure point'} />
        ...
        // ============================================================ SURROGATE (LEARNED)
        {
          ...
          content: (
            <div className="cc-doc-sec">
              ...
              <p>{es ? 'Se entrena offline (torch → ONNX) y corre EN VIVO en el navegador (onnxruntime-web). Su error held-out se reporta tal cual: 5.2% de error medio relativo, ±12.5% (σ) sobre el conjunto de prueba no visto. Crucialmente, el App muestra siempre la potencia exacta junto a la del surrogate y el error en vivo entre ambas, así el surrogate nunca se presenta como verdad sino como un estimador rápido con incertidumbre declarada.' : 'It trains offline (torch → ONNX) and runs LIVE in the browser (onnxruntime-web). Its held-out error is reported as it lands: 5.2% mean relative error, ±12.5% (σ) on the unseen test set. Crucially, the App ALWAYS shows the exact power next to the surrogate\'s and the live error between them, so the surrogate is never presented as truth but as a fast estimator with stated uncertainty.'}</p>
              ...
              <figure className="cc-fig">
                <svg viewBox="0 0 460 130" width="100%" role="img" className="cc-msvg" aria-label={es ? 'pipeline del surrogate de potencia' : 'power surrogate pipeline'}>
                  ...
                  <text className="s" x="16" y="104">{es ? 'surrogate: 5.2% medio ±12.5% σ held-out · vive en vivo (onnxruntime-web)' : 'surrogate: 5.2% mean ±12.5% σ held-out · runs live (onnxruntime-web)'}</text>
                </svg>
