          ...
          content: (
            <div className="cc-doc-sec">
              ...
              <Callout variant="honest" title={es ? 'Cinemática, no DEM' : 'Kinematic, not DEM'}>
                {es ? 'Las partículas del 3D siguen exactamente el ángulo de partida de Davis y la parábola, física publicada, no inventada. Pero es un modelo cinemático de partículas independientes: no hay colisiones partícula-partícula ni fricción de contacto (eso es DEM, un solve de N cuerpos, la mejora offline documentada). Lo que se muestra es correcto como envolvente de trayectorias, no como una simulación de cada choque.' : 'The 3D particles follow exactly the Davis departure angle and the parabola, published physics, not invented. But it is a kinematic model of independent particles: there are no particle–particle collisions or contact friction (that is DEM, an N-body solve, the documented offline upgrade). What it shows is correct as a trajectory envelope, not as a simulation of every collision.'}
              </Callout>
        ...
        // ============================================================ POWER
        {
          ...
          content: (
            <div className="cc-doc-sec">
              ...
              <p>{es ? 'El modelo de Morrell (1996) es la referencia SOTA: trata la carga como un continuo entre el pie y el hombro en vez de un cuerpo rígido, y predice la potencia a ±9.8% sobre 82 conjuntos de datos industriales. El App implementa el C-model completo (la integral de potencia sobre la forma de la carga, el término de energía cinética y la potencia sin carga; reproduce el ejemplo publicado de Erdem 2004, gross 1365 kW) y muestra su potencia neta como una curva independiente junto a Hogg–Fuerstenau en la pestaña Potencia. Al no estar calibrado a la potencia medida, tiende a sobre-estimar según la convención de densidad de carga (el residual de ~10% documentado): esa diferencia entre los dos modelos independientes es la comparación honesta, no un ajuste por construcción.' : 'Morrell\'s (1996) model is the SOTA reference: it treats the charge as a continuum between toe and shoulder rather than a rigid body, and predicts power to ±9.8% over 82 industrial data sets. The App implements the full C-model (the power integral over the charge shape, the kinetic-energy term and the no-load power; it reproduces the published Erdem 2004 worked example, gross 1365 kW) and shows its net power as an independent curve alongside Hogg–Fuerstenau on the Power tab. Being uncalibrated to measured power, it tends to overestimate under the charge-density convention (the documented ~10% residual): that difference between the two independent models is the honest comparison, not a consistency check by construction.'}</p>
              <figure className="cc-fig">
        ...
        // ============================================================ SURROGATE (LEARNED)
        {
          ...
          content: (
            <div className="cc-doc-sec">
              ...
              <p>{es ? 'Se entrena offline (torch → ONNX) y se ejecuta en vivo en el navegador (onnxruntime-web). Su error held-out se reporta tal cual: 5.2% de error medio relativo, ±12.5% (σ) sobre el conjunto de prueba no visto. El App muestra siempre la potencia exacta junto a la del surrogate y el error en vivo entre ambas, así el surrogate nunca se presenta como verdad sino como un estimador rápido con incertidumbre declarada.' : 'It trains offline (torch → ONNX) and runs live in the browser (onnxruntime-web). Its held-out error is reported as it lands: 5.2% mean relative error, ±12.5% (σ) on the unseen test set. The App always shows the exact power next to the surrogate\'s and the live error between them, so the surrogate is never presented as truth but as a fast estimator with stated uncertainty.'}</p>
              ...
              <p>{es ? 'El conjunto de entrenamiento se genera por muestreo uniforme (semilla fija) del espacio de operación (D, L, J, d, φc, densidad) y se etiqueta con el motor exacto, es decir, las etiquetas son verdad analítica, no mediciones ruidosas, así que el error held-out mide puramente la capacidad del MLP de interpolar la superficie de potencia, sin confundirse con ruido de datos. El error se evalúa siempre sobre molinos no vistos en entrenamiento (partición sin fuga), nunca sobre el conjunto de ajuste.' : 'The training set is generated by fixed-seed uniform sampling of the operating space (D, L, J, d, φc, density) and labelled with the exact engine, so the labels are analytic truth, not noisy measurements, and the held-out error purely measures the MLP\'s ability to interpolate the power surface, not confounded with data noise. The error is always evaluated on mills unseen in training (a leakage-safe split), never on the fit set.'}</p>
              <figure className="cc-fig">
                <svg viewBox="0 0 460 130" width="100%" role="img" className="cc-msvg" aria-label={es ? 'pipeline del surrogate de potencia' : 'power surrogate pipeline'}>
                  ...
                  <text className="s" x="16" y="104">{es ? 'surrogate: 5.2% medio ±12.5% σ held-out · en vivo (onnxruntime-web)' : 'surrogate: 5.2% mean ±12.5% σ held-out · runs live (onnxruntime-web)'}</text>
                </svg>
              ...
              <Callout variant="honest" title={es ? 'El exacto es la autoridad' : 'The exact engine is the authority'}>
                {es ? 'El surrogate no reemplaza el modelo de potencia: el exacto se ejecuta en vivo por defecto y es la referencia. El rol previsto del surrogate son los barridos masivos donde 10⁴ evaluaciones exactas sumarían; ninguna funcionalidad publicada ejecuta aún ese barrido, y su error (5.2% medio, ±12.5% σ) se reporta siempre. Sin victorias fabricadas.' : 'The surrogate does not replace the power model: the exact engine runs live by default and is the reference. The surrogate\'s intended role is mass sweeps where 10⁴ exact evaluations would add up; no shipped feature runs such a sweep yet, and its error (5.2% mean, ±12.5% σ) is always reported. No fabricated wins.'}
              </Callout>
        ...
        // ============================================================ OOD-AE (LEARNED)
        {
          ...
          content: (
            <div className="cc-doc-sec">
              ...
              <p>{es ? 'El puntaje OOD se umbraliza al percentil p95 de los errores de reconstrucción in-distribution held-out: un molino sobre ese umbral se marca como fuera de distribución. En datos held-out, el AE separa dentro/fuera de envolvente con un AUC de ~0.92, bueno, no perfecto, y reportado como cae.' : 'The OOD score is thresholded at the p95 of the held-out in-distribution reconstruction errors: a mill above that threshold is flagged as out-of-distribution. On held-out data, the AE separates in/out-of-envelope at an AUC of ~0.92, good, not perfect, and reported as it lands.'}</p>
              <Equation tex="\text{flag if } \text{OOD}(x) > \theta_{p95},\qquad \text{AUC}_{\text{held-out}}\approx0.92" caption={es ? 'regla de marcado y la separación held-out (dentro vs fuera de la envolvente de entrenamiento)' : 'the flag rule and the held-out separation (in vs out of the training envelope)'} />
              ...
              <figure className="cc-fig">
                <svg viewBox="0 0 460 130" width="100%" role="img" className="cc-msvg" aria-label={es ? 'autoencoder OOD: reconstrucción y umbral' : 'OOD autoencoder: reconstruction and threshold'}>
                  ...
                  <text className="s" x="280" y="64">{es ? 'marca si > umbral p95' : 'flag if > p95 threshold'}</text>
