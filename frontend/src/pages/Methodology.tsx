...
export default function Methodology() {
  ...
  return (
    <article className="page-body prose">
      ...
      <p className="lede">{es
        ? 'Velocidad crítica → la partida de Davis de partícula única + el vuelo parabólico → los regímenes → la potencia (Hogg–Fuerstenau / Morrell / Bond) → los dos modelos aprendidos (surrogate de potencia + autoencoder OOD). Clásico a SOTA, con lo exacto siempre distinguido de lo ilustrativo.'
        : 'Critical speed → the Davis single-particle departure + the parabolic flight → the regimes → the power (Hogg–Fuerstenau / Morrell / Bond) → the two learned models (power surrogate + OOD autoencoder). Classical to SOTA, with the exact always distinguished from the illustrative.'}{' '}<InlineMath tex={String.raw`\varphi_c=N/N_c`} /></p>
      ...
      <Tabs ariaLabel={es ? 'metodología' : 'methodology'} tabs={[
          ...
          content: (
            <div className="cc-doc-sec">
              ...
              <Callout variant="honest" title={es ? 'Exacto y transferible' : 'Exact and transferable'}>
                {es ? 'La velocidad crítica y φc son física exacta — valen para un molino real medido tal cual, sin calibración. C-CRITICAL (poner φc = 1 y verificar que la potencia colapsa a 0 porque toda la carga centrifuga) es un control que el motor DEBE reproducir; si no, el modelo está mal.' : 'The critical speed and φc are exact physics — they hold for a real measured mill as written, with no calibration. C-CRITICAL (set φc = 1 and check the power collapses to 0 because all the charge centrifuges) is a control the engine MUST reproduce; if not, the model is wrong.'}
              </Callout>
        ...
        // ============================================================ REGIMES
        {
          ...
          content: (
            <div className="cc-doc-sec">
              ...
              <Equation tex="f_{\text{cent}}(\varphi_c)=\frac{1}{R}\int_{r:\,\varphi_c^2 r/R\ge1} dr = \max\!\left(0,\ 1-\frac{1}{\varphi_c^2}\right)" caption={es ? 'fracción de capas centrifugando; es 0 hasta φc = 1 y crece después — el inicio cuantitativo, no una banda difusa' : 'fraction of shells centrifuging; it is 0 until φc = 1 and rises after — the quantitative onset, not a fuzzy band'} />
              ...
              <Equation tex="\theta_{\text{toe}},\ \theta_{\text{shoulder}}\ \leftarrow\ \text{env}\{\text{parábolas}\},\qquad \Delta\theta=\theta_{\text{shoulder}}-\theta_{\text{toe}}" caption={es ? 'ángulos de pie y hombro de la envolvente de trayectorias; Δθ es el ángulo de carga que entra en la potencia' : 'toe and shoulder angles from the trajectory envelope; Δθ is the charge angle entering the power'} />
              <figure className="cc-fig">
              ...
              <Callout variant="honest" title={es ? 'Bandas vs inicio exacto' : 'Bands vs exact onset'}>
                {es ? 'Las bandas de φc (0.4 / 0.65 / 0.9) son convenciones de la literatura, ÚTILES pero aproximadas — la transición real depende también del llenado y de los lifters. Lo EXACTO es la fracción de centrifugado f_cent(φc), que es 0 hasta φc = 1 por construcción. El App muestra ambas: las bandas para intuición y la fracción para el inicio cuantitativo.' : 'The φc bands (0.4 / 0.65 / 0.9) are literature conventions, USEFUL but approximate — the real transition also depends on fill and lifters. What is EXACT is the centrifuging fraction f_cent(φc), which is 0 until φc = 1 by construction. The App shows both: the bands for intuition and the fraction for the quantitative onset.'}
              </Callout>
        ...
        // ============================================================ POWER
        {
          ...
          content: (
            <div className="cc-doc-sec">
              ...
              <p>{es ? 'El modelo de Morrell (1996) es la referencia SOTA: trata la carga como un continuo entre el pie y el hombro en vez de un cuerpo rígido, y predice la potencia a ±9.8% sobre 82 conjuntos de datos industriales. El App lo muestra como una forma calibrada de cross-check junto a Hogg–Fuerstenau, para que el lector vea que los dos modelos independientes coinciden.' : 'Morrell\'s (1996) model is the SOTA reference: it treats the charge as a continuum between toe and shoulder rather than a rigid body, and predicts power to ±9.8% over 82 industrial data sets. The App shows it as a calibrated cross-check form alongside Hogg–Fuerstenau, so the reader sees that the two independent models agree.'}</p>
              <figure className="cc-fig">
        ...
        // ============================================================ SURROGATE (LEARNED)
        {
          ...
          content: (
            <div className="cc-doc-sec">
              <p>{es ? 'El primer modelo aprendido es un SURROGATE: un perceptrón multicapa (MLP) que mapea las features del molino (D, L, J, tamaño de bola, φc, densidad de carga) directamente a la potencia, entrenado para REPRODUCIR el motor analítico exacto. No reemplaza al motor — lo ACELERA para barridos masivos donde evaluar el modelo exacto miles de veces sería lento (Monte-Carlo de incertidumbre, mapas de potencia sobre grillas (φc, J)).' : 'The first learned model is a SURROGATE: a multilayer perceptron (MLP) mapping the mill features (D, L, J, ball size, φc, charge density) directly to power, trained to REPRODUCE the exact analytic engine. It does not replace the engine — it ACCELERATES it for mass sweeps where evaluating the exact model thousands of times would be slow (uncertainty Monte-Carlo, power maps over (φc, J) grids).'}</p>
              <Equation tex="\hat P=\text{MLP}_\theta(x),\qquad \theta^\*=\arg\min_\theta \tfrac{1}{N}\textstyle\sum_i \big(\text{MLP}_\theta(x_i)-P_i^{\text{exacto}}\big)^2" caption={es ? 'el surrogate se entrena para reproducir la potencia del motor EXACTO (x = features del molino; P = potencia)' : 'the surrogate is trained to reproduce the EXACT engine power (x = mill features; P = power)'} />
              ...
              <Equation tex="\varepsilon=\frac{|\hat P-P^{\text{exacto}}|}{P^{\text{exacto}}},\qquad \text{reportado en vivo en la pestaña What-if}" caption={es ? 'el error relativo del surrogate vs el exacto, mostrado en vivo donde el surrogate es la herramienta' : 'the surrogate\'s relative error vs the exact, shown live where the surrogate is the tool'} />
              <p>{es ? 'Por qué un MLP y no algo más simple: la potencia es no-lineal en φc y J (el término de llenado, el escalado con D), y un surrogate suave que captura esa superficie en microsegundos hace posible el Monte-Carlo interactivo. Las features son cantidades físicas ya interpretables, no datos crudos, así que las salidas se pueden auditar contra la intuición.' : 'Why an MLP and not something simpler: power is non-linear in φc and J (the fill term, the D-scaling), and a smooth surrogate capturing that surface in microseconds makes interactive Monte-Carlo possible. The features are already-interpretable physical quantities, not raw data, so the outputs can be audited against intuition.'}</p>
              <figure className="cc-fig">
              ...
              <Callout variant="honest" title={es ? 'El exacto es la autoridad' : 'The exact engine is the authority'}>
                {es ? 'El surrogate NO reemplaza el modelo de potencia: el exacto corre en vivo por defecto y es la referencia. El surrogate existe para los barridos masivos donde 10⁴ evaluaciones exactas serían lentas, y su error (~±12.5%) se reporta siempre. Sin victorias fabricadas.' : 'The surrogate does NOT replace the power model: the exact engine runs live by default and is the reference. The surrogate exists for the mass sweeps where 10⁴ exact evaluations would be slow, and its error (~±12.5%) is always reported. No fabricated wins.'}
              </Callout>
        ...
        // ============================================================ OOD-AE (LEARNED)
        {
          ...
          content: (
            <div className="cc-doc-sec">
              ...
              <Equation tex="\text{marca si } \text{OOD}(x) > \theta_{p99},\qquad \text{AUC}_{\text{held-out}}\approx0.92" caption={es ? 'regla de marcado y la separación held-out (dentro vs fuera de la envolvente de entrenamiento)' : 'the flag rule and the held-out separation (in vs out of the training envelope)'} />
